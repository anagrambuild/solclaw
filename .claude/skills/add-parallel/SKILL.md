...
Adds Parallel AI MCP integration to SolClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses SolClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. SolClaw already set up and running
...
- Check task runs: `tail -f logs/solclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.solclaw` (macOS) or `systemctl --user restart solclaw` (Linux)
