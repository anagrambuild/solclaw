...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.solclaw.plist`) in the `EnvironmentVariables` dict if using launchd.
...
>
> Check logs: `tail -f logs/solclaw.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/solclaw.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.solclaw.plist && launchctl load ~/Library/LaunchAgents/com.solclaw.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart solclaw` (Linux)
