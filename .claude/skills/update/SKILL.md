---
name: update
description: "Update SolClaw from upstream. Fetches latest changes, merges with your customizations and skills, runs migrations. Triggers on \"update\", \"pull upstream\", \"sync with upstream\", \"get latest changes\"."
---
...
```bash
test -d .solclaw && echo "INITIALIZED" || echo "NOT_INITIALIZED"
```
...
**Rollback:** If something goes wrong after applying but before cleanup, the backup is still in `.solclaw/backup/`. Run:
```bash
