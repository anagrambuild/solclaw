---
name: x-integration
description: X (Twitter) integration for SolClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** SolClaw v1.0.0. Directory structure may change in future versions.
...
1. **SolClaw is installed and running** - WhatsApp connected, service active
...
# Linux: systemctl --user restart solclaw
# Verify: launchctl list | grep solclaw (macOS) or systemctl --user status solclaw (Linux)
```
...
| `logs/solclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into SolClaw, make the following modifications:
...
**Verify success:**
```bash
launchctl list | grep solclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status solclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/solclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/solclaw.log | tail -20
```
