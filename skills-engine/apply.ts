...
import { SOLCLAW_DIR } from './constants.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  const statePath = path.join(projectRoot, SOLCLAW_DIR, 'state.yaml');
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, SOLCLAW_DIR, 'base', resolvedPath);
