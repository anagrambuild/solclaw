...
async function runAgent(
  ...
  const model = group.model || DEFAULT_MODEL || undefined;
...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info(`SolClaw running (trigger: @${ASSISTANT_NAME})`);
...
async function main(): Promise<void> {
  ...
  startIpcWatcher({
    ...
    updateGroupModel: (groupFolder, model) => {
      ...
      setRegisteredGroup(jid, nextGroup);
      logger.info(
        { jid, groupFolder, model: normalized || 'default' },
...
if (isDirectRun) {
  main().catch((err) => {
    logger.error({ err }, 'Failed to start SolClaw');
    process.exit(1);
  });
}
