...
beforeEach(() => {
  ...
  deps = {
    ...
    updateGroupModel: (groupFolder, model) => {
      ...
      const updated = { ...group, model: normalized };
      if (!normalized) delete updated.model;
      groups[jid] = updated;
...
describe('set_group_model authorization', () => {
  it('main group can set another group model', async () => {
    await processTaskIpc(
      {
    ...
    expect(group?.model).toBe('claude-opus-4-1');
  });

  it('non-main group cannot set another group model', async () => {
    await processTaskIpc(
      {
        type: 'set_group_model',
        groupFolder: 'main',
  ...
  it('group can clear its own model override', async () => {
    await processTaskIpc(
      {
        type: 'set_group_model',
        groupFolder: 'other-group',
